...
def kolya_roast():
    ...
    variants = [
        ...
        (
            "Либераха пойман на очередной "
            "доставке полупиздежа 📦"
        ),
        ...
        (
            "Николай, ну ты хуесос "
            "информационного пространства 😄 "
            "Опять привёз свежак."
        ),

        (
            "Коля-либераха на посту: "
            "новость доставлена, "
            "здравый смысл в пути."
        ),
        ...
        (
            "Коля-либераха, ну хоть раз "
            "принеси новость без запаха наброса 😄"
        ),

        (
            "Так, пацаны, Коля снова "
            "притащил интернет в пакете."
        ),
    ]
...
def source_keyboard(
    ...
    # Две ссылки в одну строку.
    # Поэтому длинные URL не занимают полэкрана.
    for i in range(
        0,
        len(buttons),
        2
...
def handle_message(
    ...
    #
    # Сначала проверяем:
    # человек написал "Проверь" или нет.
    #
    # Поэтому если:
    #
    # Коля кидает новость
    # Рома отвечает "Проверь"
    #
    # бот НОРМАЛЬНО фактчекает,
    # а не стебёт Колю вместо проверки.
    ...
    # При AUTO_CHECK=false
    # этот блок не запускается.
    #
    # То есть остальные могут
    # спокойно кидать новости,
    # а бот молчит до слова "Проверь".
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # Если Telegram говорит 409,
            # значит одновременно работают
            # два экземпляра бота.
