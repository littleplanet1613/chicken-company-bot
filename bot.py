...
Сначала пойми, что в сообщении
является буквальным фактом,
...
которые стоят за подачей.

Не придумывай скрытый смысл,
если он не следует
из текста достаточно ясно.
...
- если фраза вроде
«высокоточные обломки»
является иронической,

не утверждай в ответе,
что существуют
«высокоточные обломки»;
...
это само по себе
НЕ делает новость ложной

и НЕ требует 🟠;

- 🟠 используй только,
если реальный контекст,
причинность,
дата,
объект
или смысл
...
Сначала определи тип подачи:

- factual_news — обычное фактическое сообщение;
- sarcasm — очевидный сарказм/ирония;
- opinion — мнение или оценка;
- advertising — реклама/маркетинговая гипербола;
- mixed — смесь фактов и небуквальной подачи.


Дальше сделай поисковый план.
...
- запросы строй
по реальным
claims/implied_claims,

а не по
саркастической фразе;
...
def normalize_final_answer(
    ...
    if not found:
        ...
        return (
            ...
            f"Уверенность: "
            f"{min(fallback_confidence, 5)}/10"
        )
...
{plan.get("content_type", "unknown")}


БУКВАЛЬНЫЕ ПРОВЕРЯЕМЫЕ УТВЕРЖДЕНИЯ:

{claims_text or "- не выделены"}


ПОДРАЗУМЕВАЕМЫЙ ФАКТИЧЕСКИЙ СМЫСЛ:

{implied_text or "- не выделен"}


НЕБУКВАЛЬНЫЕ ФРАЗЫ / САРКАЗМ / ИРОНИЯ:

{nonliteral_text or "- не выделены"}
...
небуквальную фразу
как физический
или технический факт;

- не повторяй
саркастическую характеристику
...
сарказм сам по себе
не делает пост ложным;

- 🟠 ставь только
при реально искажающем
контексте,

а не просто
из-за шутливой подачи;

- если центральный тезис
подтверждён,

но второстепенная деталь
не найдена,
...
def handle_message(message):
    ...
    # GROUP AUTO CHECK
    #
    # Только ссылка или пересланный пост.
    # Обычная переписка бот не триггерит.
    # =====================================================
