...
PLANNER_SYSTEM = """
Ты поисковый планировщик для фактчекера.

Твоя задача —
понять смысл новости
и сделать хороший план веб-поиска.
...
Названный или прямо приложенный
домен первоисточника:
...
- если в новости
несколько разных утверждений,
запросы должны покрыть
разные утверждения;
...
{{
  ...
  "freshness": "normal",
  "document": false,
  "notes": "короткая техническая заметка"
}}
""".strip()
...
ПЛАНИРОВЩИК ВЫДЕЛИЛ УТВЕРЖДЕНИЯ:

{claims_text or "- не выделены"}


СУЩНОСТИ И ПОЛЕЗНЫЕ АЛИАСЫ:
...
Если одна второстепенная деталь
не подтверждена,
но центральный тезис подтверждён,
не превращай это автоматически
в ложь.
...
def factcheck(
    ...
    print(
        (
            ...
            + (
                f" | claims="
                f"{len(plan.get('claims', []))}"
            )
            + (
                f" | preferred="
                f"{preferred_domain or '-'}"
            )
        ),
        flush=True,
    )
...
def friendly_error(exc):
    ...
    if "GROQ_429" in text:
        ...
        return (
            "Groq упёрся в лимит. "
            "V10 обычно делает 2 вызова: "
            "один на разбор новости "
            "и один на финальный вердикт. "
            "Попробуй чуть позже."
        )
...
def main():
    ...
    print(
        (
            "Chicken Company bot V10 started; "
            "ai_planner=True; "
            "normal_groq_calls=2; "
            "searches=max3; "
            "nikolai_roasts=OFF; "
