...
Тебе передают:
1. текст новости;
2. результаты веб-поиска Tavily.

Проверяй только по найденным данным.
Ничего не выдумывай.

СНАЧАЛА мысленно определи масштаб события:
...
Он нужен тебе для правильной оценки источников.
...
ОТСУТСТВИЕ публикации в крупных СМИ
...
Для локальных событий нормальными
и важными источниками являются:
...
Несколько местных СМИ могут подтверждать
локальное событие.
...
для масштаба события источниками.
...
"В Новороссийске будут судить студента..."

Если несколько городских СМИ пишут
об этом, но сообщение суда пока
не найдено:
...
и объяснить, что местные публикации есть,
но официального подтверждения
пока недостаточно.

Если найден официальный сайт суда,
прокуратуры, МВД или СК,
подтверждающий событие:
...
"Крупные СМИ об этом не сообщают"

как аргумент в пользу ложности
локальной новости.
...
def source_priority(
    ...
    # не считаем плохими.
    # Просто ставим после официальных.
    return 3
