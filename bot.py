...
# Extract: читаем исходную ссылку + лучшие найденные страницы.
MAX_EXTRACT_URLS = 5
MAX_EXTRACT_CHARS_PER_SOURCE = 4500
MAX_SEARCH_SNIPPET_CHARS = 900

# Если прислали почти только URL, сначала читаем исходную страницу,
# чтобы по её тексту сформировать нормальные RU + EN запросы.
...
Смотри на содержание источника, а не только на его заголовок.
...
def safe_tavily_extract_urls(urls):
    """
    Extract улучшает проверку,
    но ошибка чтения отдельного сайта
    не должна ломать весь фактчек.
    """
