...
2. Мысленно разбивай составную новость
на факты,
...
но неверная дата,
старый контекст
или искажённая цитата.

8. ⚪ ХУЙ ПОЙМЁШЬ ПОКА —
если данных реально недостаточно.
...
def build_search_plan(
    ...
    prompt = f"""
Создай максимум ДВА первичных
поисковых запроса для фактчека.
...
- если тема иностранная
или цитата переведена —
оба запроса желательно
на языке оригинала;

- два запроса должны быть
разными по формулировке;

- для иностранной цитаты
ищи смысл и синонимы,
...
- latest_update разрешён
только если новость реально содержит:
...
def build_third_query(
    ...
    prompt = f"""
Первые поиски для фактчека слабые.
...
{plan["cleaned_news"][:3000]}
""".strip()

    data = parse_json(
        groq_text(
            (
                "Ты создаёшь один последний "
                "точный веб-запрос."
            ),
...
def main():
    ...
    print(
        (
            "Chicken Company bot V7 started; "
